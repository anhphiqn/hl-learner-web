...
import { getEnrolledCourses, deleteCourse } from '../actions/courseActions';
...
class ListMyCourse extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchTerm: '',
  ...
  componentDidMount() {
    console.log('ListMyCourse componentDidMount');
    this.doSearch();
  }
...
*/
  ...
  doSearch() {
    ...
    this.props.getEnrolledCourses(this.state.searchTerm, this.props.user.id, this.props.user.email);
  }
    ...
    if (loading) {
      ...
      if (this.props.courses) {
        courseContent = this.props.courses.map(course => (
          <tr key={course._id}>
            <td><Link to={`/mycourse/${course._id}`}>{course.code}</Link></td>
            <td>{course.name}</td>
            <td>
            <button
...
const mapStateToProps = state => {
  console.log('mapStateToProps  ListMyCourse');
  return {
...
export default connect(mapStateToProps, { getEnrolledCourses, deleteCourse })(
  ListMyCourse
);

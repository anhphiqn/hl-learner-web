...
// enrollCourse
export const unEnrollCourse = (courseUnEnrollData, callback) => dispatch => {
  dispatch(setCourseLoading());

  const serviceUrl = process.env.REACT_APP_COURSE_SERVICE_URL;
  const url = `${serviceUrl}/course/unenroll`; 
  console.log(url);
  axios
    .post(url, courseUnEnrollData)
    .then(res =>{
      dispatch({
        type: GET_COURSE,
        payload: res.data
    ...
    .catch(err => {
      console.log('unEnrollCourse error ', err);
      dispatch({
        type: GET_ERRORS,
        payload: err.response.data
      });
...
// Search unenrolled courses
export const getUnEnrolledCourses = (searchTerm, learnerId, userEmail) => dispatch => {
  dispatch(setCourseLoading());
  const serviceUrl = process.env.REACT_APP_COURSE_SERVICE_URL
  searchTerm = !searchTerm ? '*' : searchTerm; 
  const url = `${serviceUrl}/course/searchUnEnrolled/${searchTerm}/${learnerId}/${userEmail}`;
  console.log(url);
...
export const getEnrolledCourses = (searchTerm, learnerId, userEmail) => dispatch => {
  dispatch(setCourseLoading());
  const serviceUrl = process.env.REACT_APP_COURSE_SERVICE_URL
  searchTerm = !searchTerm ? '*' : searchTerm; 
  const url = `${serviceUrl}/course/searchEnrolled/${searchTerm}/${learnerId}/${userEmail}`;
  console.log(url);
